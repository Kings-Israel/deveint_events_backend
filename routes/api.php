<?php

use Illuminate\Http\Request;
use Illuminate\Support\Facades\Route;

/*
|--------------------------------------------------------------------------
| API Routes
|--------------------------------------------------------------------------
|
| Here is where you can register API routes for your application. These
| routes are loaded by the RouteServiceProvider within a group which
| is assigned the "api" middleware group. Enjoy building your API!
|
*/

// Route::middleware('auth:sanctum')->get('/user', function (Request $request) {
//     return $request->user();
// });

Route::middleware(['auth:sanctum'])->group(function () {
    Route::get('/user', function(Request $request) {
        return $request->user();
    });
    Route::get('/events/me/', 'EventController@myEvents');
    Route::get('/events/all/', 'EventController@allEvents');
    Route::post('/event/add', 'EventController@add');
    Route::post('/event/update', 'EventController@update');
    Route::delete('/event/delete/{id}', 'EventController@delete'); 
    Route::post('/event/book', 'EventController@book');
    Route::get('/events/booked/', 'EventController@bookedEvents');
    Route::delete('/events/booked/delete/{id}', 'EventController@deleteBookedEvent');
});
